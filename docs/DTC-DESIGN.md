...
Version: v10.7 · Last updated: 2026-08-25
...
## Open items
- [x] **4 new merchants not yet in the live Merchants tab** — added
      2026-08-04: R98 Driving Academy (Services), Santigwar (Food &
      Dining), Southern Crate (Services), Rebel Coffee (Food & Dining).
      **Resolved**: confirmed live in the 2026-08-20
      `assets/merchants/live-snapshot.json` fetch — all 4 are now in the
      backend Sheet's Merchants tab and served by the real `getPartners`
      endpoint, not just this repo's fallback JSON. Checkbox was simply
      never updated when this actually landed.
...
- [x] **`getPartners` double-encoding bug, found 2026-07-24, root cause
      ...
      string is in the cell, faithfully and correctly. **Resolved**:
      the 2026-08-20 `live-snapshot.json` fetch shows the cell now reads
      "White Bean Cafe" (unaccented) — someone retyped it directly in
      the Sheet at some point between 2026-08-04 and 2026-08-20, and the
      mojibake is gone. This repo's `partners.json` still spells it
      "White Bean Café" (accented) — a cosmetic spelling mismatch against
      the live Sheet, not a bug; leave as-is unless the club confirms
...
- [ ] **`Code.gs (v11)` written 2026-08-24/25, NOT YET DEPLOYED.** Adds
      Rurok bulletin automation (`syncRurokIssues`, `getRurokIssues_`,
      the new `?action=rurokIssues`, plus `enforceHeyzineFlipbookCap_`
      for the 5-flipbook cap — see docs/RUROK-DESIGN.md's "Automation"
      section) to the same shared Web App/Sheet — unrelated to any DTC
      logic, which is unchanged from v9's behavior, but bumps the file
      version since it's the same script. Manual setup steps: paste
      into the editor, run `setupWorkbook()` to create the new
      `RurokIssues` tab (8 columns, including `pdf_url`), add a
      `HEYZINE_API_KEY` Script Property, run `syncRurokIssues()` once,
      add its daily time-driven trigger, redeploy as a new Web App
      version. Nobody has done any of this yet — DTC's own
      `?action=verify/partners` behavior is completely unaffected
      either way, so there's no urgency tied to DTC itself, but
      `/rurok/`'s live fetch will keep failing over to its static JSON
      fallback until this is done.
